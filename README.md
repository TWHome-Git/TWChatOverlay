...
- Windows 11 · 16코어 환경, Release 빌드 실측 기준 (CPU는 프로세스 CPU 시간 60초 샘플링, 메모리는 작업관리자와 같은 Private Working Set).
- 폭주 수치는 실제 게임 로그 형식의 채팅(일반·클럽·외치기 팝업 포함)을 해당 속도로 재생해 측정한 값으로, 초당 30줄은 실전 최대 폭주를 크게 웃도는 조건입니다.
