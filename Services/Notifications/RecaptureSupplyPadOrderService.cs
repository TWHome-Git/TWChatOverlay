...
namespace TWChatOverlay.Services
{
    ...
    /// 던전 밖에서도 같은 글이 흘러갈 수 있으므로 <see cref="AppServices.Get<RecaptureSupplyAlertService>().IsInRun"/>일 때만 본다.
    /// </summary>
    public sealed class RecaptureSupplyPadOrderService
    {
        ...
        private readonly PadPattern[] Patterns =
        {
        ...
        private RecaptureSupplyPadOrderWindow? _window;

        public void Observe(string formattedText)
        {
            if (string.IsNullOrWhiteSpace(formattedText))
                return;
            if (!AppServices.Get<RecaptureSupplyAlertService>().IsInRun)
        ...
        private RecaptureSupplyPadOrderWindow EnsureWindow(ChatSettings? settings)
        {
        ...
        private void ApplyStoredBounds(RecaptureSupplyPadOrderWindow window, ChatSettings settings)
        {
