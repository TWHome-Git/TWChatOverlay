...
namespace TWChatOverlay.Services
{
    ...
    {
        // Hardcoded ignore rules (no external json/remote dependency).
        private static readonly HashSet<string> NormalIgnoredMessages = new(StringComparer.Ordinal)
        {
            "금화 주머니를 획득 했습니다.",
            "지역 보상상자를 열었습니다.",
            "이공간 보물상자를 열었습니다.",
        ...
        {
            new(@"^(SP|MP|Fever|HP)가\s*\d+%\s*회복되었습니다\.?$", RegexOptions.Compiled),
            new(@"^체력이\s*\d+%\s*회복되었습니다\.?$", RegexOptions.Compiled)
        };
        ...
        public static bool IsIgnoredNormalMessage(string message)
        {
            ...
            if (NormalIgnoredMessages.Contains(message))
                return true;
